...
public class DragSortListView extends ListView {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            canvas.translate(mFloatLoc.x,
                    mFloatLoc.y - mFloatViewManager.getPaddingTop());
    ...
    private boolean updatePositions() {
        ...
        if (mAnimate) {
            ...
            if (mFloatViewMid < slideEdgeTop) {
                ...
                mSlideFrac = 0.5f * (slideEdgeTop - mFloatViewMid)
                        / slideRgnHeight;
                // Log.d("mobeta",
    ...
    private class SmoothAnimator implements Runnable {
        ...
        public void run() {
            ...
            float fraction = (SystemClock.uptimeMillis() - mStartTime)
                    / mDurationF;
                ...
                if (item != null) {
                    if (mSecondChildHeight == -1) {
                        mSecondChildHeight = getChildHeight(mSecondPos, item,
                                false);
                        mSecondStartBlank = item.getHeight()
                                - mSecondChildHeight;
                    }
    ...
    private void measureFloatView() {
        if (mFloatView != null) {
            ...
            mFloatViewHeight = mFloatView.getMeasuredHeight()
                    - mFloatViewManager.getPaddingTop()
                    - mFloatViewManager.getPaddingBottom();
    ...
    public void moveCheckState(int from, int to, SparseBooleanArray cip) {
        int rangeStart = from;
        int rangeEnd = to;
        if (to < from) {
            rangeStart = to;
        ...
        if (from < to) {
            for (int i = 0; i != runCount; i++) {
                cip.put(rotate(runStart[i], -1, rangeStart, rangeEnd), true);
                cip.put(rotate(runEnd[i], -1, rangeStart, rangeEnd), false);
            }
        } else {
            for (int i = 0; i != runCount; i++) {
                cip.put(runStart[i], false);
                cip.put(runEnd[i], true);
            }
        }
    }

    public SparseBooleanArray moveCheckState(int from, int to,
            ArrayList<Integer> removeIndexs) {
        SparseBooleanArray cip = new SparseBooleanArray();
        for (int i = 0; i < removeIndexs.size(); i++) {
            cip.put(i, true);
        }
        moveCheckState(from, to, cip);
        removeIndexs.clear();
        int size = cip.size();
    ...
    public void removeCheckState(int position) {
        ...
        for (int i = 0; i != runCount; i++) {
            if (!(runStart[i] == position || (runEnd[i] < runStart[i] && runEnd[i] > position))) {
                ...
                setItemChecked(rotate(runStart[i], -1, position, rangeEnd),
                        true);
            }
